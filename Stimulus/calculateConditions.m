...
if any(ismember(TRIALINFO.intergration,0))
    ...
    con0AC = [repmat(con0AS,length(AUDITORY.coherence),1),...
        sortrows(repmat(AUDITORY.coherence',size(con0AS,1),1))];
    ...
    % visual only
    con0T0 = cat(2,con0V,num2cell(nan(size(con0V,1),size(con0AC,2))));
    
    % auditory only
    con0T1 = cat(2,num2cell(nan(size(con0AC,1),size(con0V,2))),con0AC);
...
if any(ismember(TRIALINFO.intergration,1))
    ...
    con1AC = [repmat(con1AS,length(AUDITORY.coherence),1),...
        sortrows(repmat(AUDITORY.coherence',size(con1AS,1),1))];
    ...
    % visual only
    con1T0 = cat(2,con1V,num2cell(nan(size(con1V,1),size(con1AC,2))));
    
    % auditory only
    con1T1 = cat(2,num2cell(nan(size(con1AC,1),size(con1V,2))),con1AC);
    
    % both provided
    con1T2 = cat(2,con1AC(:,1:3),con1AC);
